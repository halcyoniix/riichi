...
const checkAllowed = (o, allowed) => {
  for (let v of o.hai) if (!allowed.includes(v)) return false;
  for (let v of o.furo)
    for (let vv of v) if (!allowed.includes(vv)) return false;
  return true;
};
...
const YAKU = {
  ...
  大七星: {
    ...
    check: (o) => {
      let allow = ["1z", "2z", "3z", "4z", "5z", "6z", "7z"];
      return checkAllowed(o, allow) && YAKU["二盃口"].check(o);
    },
  },
  大車輪: {
    yakuman: 1,
  ...
  断么九: {
    han: 1,
    check: (o) => {
      for (let v of o.furo) if (!o.allowKuitan && v.length !== 2) return false;
      let allow = [
        "2m",
        "3m",
        "4m",
