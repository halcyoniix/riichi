  ...
  const syanten = (hai_arr) => {
    ...
    const search = (arr, is_jihai = false) => {
      ...
      let tmp2 = [0, 0, 0];
      {
        ...
        let tmp_alone = 0;
        for (let i = 0; i < 9; i++) {
          if (arr2[i] === 0 || arr2[i] === undefined) continue;
          if (!is_jihai) {
  ...
  const checkChi = (hai_arr, isUniverse = false) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (let ii = 0; ii < 9; ii++) {
        let list = [];
        let stat = false;
        if (hai_arr[i][ii - 2] > 0 && hai_arr[i][ii - 1] > 0) {
          list.push(ii - 1 + "" + ii + MPSZ[i]);
          stat = true;
        }
        if (hai_arr[i][ii - 1] > 0 && hai_arr[i][ii + 1] > 0) {
          list.push(ii + "" + (ii + 2) + MPSZ[i]);
          stat = true;
        }
        if (hai_arr[i][ii + 1] > 0 && hai_arr[i][ii + 2] > 0) {
          list.push(ii + 2 + "" + (ii + 3) + MPSZ[i]);
          stat = true;
        }
        if (stat) {
          res.count++;
  ...
  const checkPon = (hai_arr) => {
    ...
    for (let i = 0; i < 4; i++) {
      for (let ii = 0; ii < 9; ii++) {
        if (hai_arr[i][ii] > 1) {
          map[ii + 1 + MPSZ[i]] = {
            minkan: hai_arr[i][ii] > 2,
            ankan: hai_arr[i][ii] > 3,
          };
          res.count++;
        }
      }
